...
5. Devil's Advocate (핵심 발견만)
   - 핵심 발견 1-2개에 "이게 틀리다면?" 질문
   - 반증 쿼리 1회 실행
   - 반증 발견 → 흥미 +0.3, 새 탐험 기회!
...
### 2.6 Devil's Advocate (가벼운 반증)

**핵심 발견에 대해 한 번만 의심:**

```
발견: "LLMLingua-2는 품질 손실 없이 50% 압축 가능"

Devil's Advocate:
  Q: "이게 틀리다면 어떤 증거가 있을까?"
  → 반증 쿼리: "LLMLingua limitations" or "prompt compression quality loss"
  → WebSearch 1회 실행
```
...
| 반증 결과 | 액션 |
|----------|------|
| 반증 발견 | 흥미 +0.3, 새 hole 생성 가능 ("한계점 탐구") |
| 반증 없음 | 신뢰도 ✓✓ 승격, 그대로 진행 |
| 부분 반증 | 조건 명시 ("X 상황에서만 유효") |

**규칙:**
- 모든 발견 아님, **핵심 발견 1-2개만** (탐험 흐름 유지)
- 반증 쿼리는 **1회만** (과도한 검증 방지)
- 반증도 흥미로운 탐험 기회! (수렴이 아닌 확장)
